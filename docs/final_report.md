...
- [结题报告](#结题报告)
    ...
    - [项目架构](#项目架构)
        ...
        - [Hardware](#hardware)
            - [准备工作](#准备工作)
            - [树莓派BCM2837设备驱动（`os/pi/`）](#树莓派bcm2837设备驱动ospi)
        - [File System](#file-system)
            - [FAT32 文件系统](#fat32-文件系统)
            - [代码结构](#代码结构)
        - [Interrupt & Exception](#interrupt--exception)
            - [ARM 对 Interrupt & Exception 的分类](#arm-对-interrupt--exception-的分类)
            - [exception 处理](#exception-处理)
            - [syscall](#syscall)
            - [interrupt](#interrupt)
        - [Process](#process)
            - [Stack](#stack)
            - [State](#state)
            - [Scheduler](#scheduler)
            - [难点：第一个进程的建立](#难点第一个进程的建立)
            - [具体实现](#具体实现)
        - [Shell](#shell)
            - [简介](#简介)
            - [Shell 的主要组成部分](#shell-的主要组成部分)
    - [Get Started](#get-started)
        - [软硬件环境](#软硬件环境)
            - [硬件](#硬件)
            - [软件](#软件)
        - [安装 aarch64-none-elf 工具](#安装-aarch64-none-elf-工具)
        - [克隆项目](#克隆项目)
        - [安装 ttywrite 工具](#安装-ttywrite-工具)
        - [在 Micro SD 中放入启动必需文件](#在-micro-sd-中放入启动必需文件)
        - [连接设备](#连接设备)
        - [运行](#运行)
    - [前景展望](#前景展望)
    - [总结分析](#总结分析)
        - [优点和收获](#优点和收获)
            - [贴近 OS 课程本身](#贴近-os-课程本身)
            - [初步掌握了 Rust 语言](#初步掌握了-rust-语言)
        - [缺点和不足](#缺点和不足)
            - [对课程所作的拓展有限](#对课程所作的拓展有限)
            - [利用 Rust 语言的工程能力不足](#利用-rust-语言的工程能力不足)
    - [参考资料](#参考资料)
...
到最后，我们对课程所作的拓展主要是对进程管理的完善和对 Shell 功能的扩充。我们为进程管理添加了`kill`功能；我们的 Shell 程序支持`echo`、`pwd`、`cd`、`ls`、`cat`、`allocator`（显示当前内存分配情况）、`sleep`（是一个系统调用）命令，还支持左右方向键移动光标、上下方向键在历史记录中切换。
