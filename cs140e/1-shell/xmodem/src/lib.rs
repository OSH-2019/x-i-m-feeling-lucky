...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    ///
    /// An error of kind `UnexpectedEof` is returned if `buf.len() < 128`.          //ok    
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "expected length: 128"));
        }
        ...
        match self.read_byte(true)? {
            EOT => {
                self.write_byte(NAK)?;
                self.expect_byte(EOT, "expected EOT for ending")?;
                self.write_byte(ACK)?;
                self.started = false;
                Ok(0)
            }
            SOH => {
                ...
                self.expect_byte_or_cancel(!packet, "packet number checsum wrong")?;
                ...
                if self.read_byte(false)? != checksum {
                    self.write_byte(NAK)?;
                    Err(io::Error::new(io::ErrorKind::Interrupted, "checksum wrong"))
                } else {
                    self.write_byte(ACK)?;
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        if buf.len() < 128 && buf.len() != 0 {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "expected length: 0 or 128"));
        }
        ...
        match self.read_byte(true)? {
            ...
            NAK => {
                Err(io::Error::new(io::ErrorKind::Interrupted, "data corrupted"))
            }
            _ => {
                Err(io::Error::new(io::ErrorKind::InvalidData, "expected ACK or NAK"))
            }
        }
    }
